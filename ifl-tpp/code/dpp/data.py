...
def load_dataset_min(name, normalize_min_max=False, log_mode=True, min_val=0):
    """Load dataset."""
    if not name.endswith('.npz'):
        name += '.npz'
    loader = dict(np.load(dataset_dir / name, allow_pickle=True))
    try:
        ...
        arrival_times = loader['arr_0']
    #marks = np.concatenate(loader.get('marks', None))
    deltas = np.ediff1d(np.concatenate(arrival_times))
    #marks = marks[deltas >= min_val]
    ...
    #num_classes = len(set([x for s in marks for x in s])) if marks is not None else 1
    return SequenceDataset(deltas, marks=None, num_classes=1, log_mode=log_mode)
